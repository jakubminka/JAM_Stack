(function() {
    ...
    if (node.className.indexOf("closed") === -1) {
      if (node.tagName.toLowerCase() === "form") {
        ...
        var more = document.querySelector(
          "#" + node.id + " .collapsible:first-child:not(.closed) .more-btn"
        );
        if (more) {
          node = more;
        } else if (form) {
  ...
  var sortItem = function(id) {
    ...
    var nodes = document.querySelectorAll(
      '#shopping-list-items .item-view input[type="checkbox"]'
    );
  ...
  var setAria = function(hide) {
    if (hide) {
      ...
      var nodes = document.querySelectorAll(
        'header [role="button"], main [role="button"]:not([aria-hidden="true"]), main button:not([aria-hidden="true"]), main input:not([aria-hidden="true"]), main [type="checkbox"]:not([aria-hidden="true"])'
      );
      for (var i = 0; i < nodes.length; i++) {
      ...
      var hiddennodes = document.querySelectorAll(
        'main [tab-index="-1"], header [tab-index="-1"]'
      );
  ...
  shopper.toggle = function(node, event) {
    ...
    if (typeof node === "string") {
      ...
      var inputs = document.querySelectorAll(
        "#" + node + " .collapsible input[placeholder]"
      );
      ...
      for (var k = 0; k < buttons.length; k++) {
        buttons[k].setAttribute(
          "aria-hidden",
          buttons[k].getAttribute("aria-hidden") !== "true"
        );
      }
    } else {
    ...
    for (var setting in shopper.settings) {
      if (form.elements.hasOwnProperty(setting)) {
        var input = document.querySelector(
          "form#shopping-list-settings [name=" + setting + "]"
        );
  ...
  shopper.settings = function(event) {
    ...
    for (var i = 0; i < elements.length; i++) {
      if (elements[i].tagName.toLowerCase() !== "button") {
        ...
        doc[elements[i].name] = shopper.settings[elements[i].name] =
          elements[i].value;
      }
    }
  ...
  shopper.sync = function(callback) {
    ...
    if (shopper.settings.remoteDB) {
      document.body.className = document.body.className
        .replace("shopping-list-error-sync", "")
        .trim();
      ...
      var change = function(err, docs) {
        if (err) {
          ...
          if (
            document.body.className.indexOf("shopping-list-error-sync") !== -1
          ) {
            document.body.className = document.body.className
              .replace("shopping-list-error-sync", "")
              .trim();
